#!/bin/bash
PROJECT_NAME="dy_cavity_relocker_2"

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

TARGET_DIR="$ROOT_DIR/pynq_overlays"
...
inotifywait -q -m -r --exclude "pynq_overlays" -e close_write --format '%w%f' "$BUILD_ROOT" | while read NEW_BIT; do
	if [[ "$NEW_BIT" == *".bit" ]]; then 
		echo "BITSTREAM DETECTED: $NEW_BIT"
		sleep 2
		...
		LATEST_HWH=$(find "$BUILD_ROOT" -type f -name "*.hwh" -printf '%T@ %p\n' | sort -n | tail -1 | cut -d' ' -f2-)
